...
pub struct OpCode {
    ...
    pub operation: fn(OpCodeArgs, Cpu),
}

impl OpCode {
    /// Contruct a new OpCode given its u16 opcode, its dissassembly string, and its operation delegate
    pub fn new(opcode: u16, args: OpCodeArgs, disasm_str: String, operation: fn(OpCodeArgs, Cpu)) -> OpCode {
        OpCode {
            opcode: opcode,
            args: args,
    ...
    fn opcode_sys(args: OpCodeArgs, cpu: Cpu) {
        println!("SYS opcode called.");
    }

    /// 0x00E0
    /// "CLS" opcode. Clears the display.
    fn opcode_cls(args: OpCodeArgs, cpu: Cpu) {

    }

    /// 0x00EE
    /// "RET" opcode. Returns from a subroutine.
    fn opcode_ret(args: OpCodeArgs, cpu: Cpu) {

    }

    /// 0x1nnn
    /// "JP addr" opcode. Jumps to a specified address.
    fn opcode_jp_addr(args: OpCodeArgs, cpu: Cpu) {

    }

    /// 0x2nnn
    /// "CALL addr" opcode. Calls the subroutine at the given address.
    fn opcode_call_addr(args: OpCodeArgs, cpu: Cpu) {

    }

    /// 0x3xkk
    /// "SE Vx, byte" opcode. Skip next instruction if Vx = kk.
    fn opcode_se_vx_byte(args: OpCodeArgs, cpu: Cpu) {

    }

    /// 0x4xkk
    /// "SNE Vx, byte" opcode. Skip next instruction if Vx != kk.
    fn opcode_sne_vx_byte(args: OpCodeArgs, cpu: Cpu) {

    }

    /// 0x5xy0
    /// "SE Vx, Vy" opcode. Skip next instruction if Vx = Vy.
    fn opcode_se_vx_vy(args: OpCodeArgs, cpu: Cpu) {

    }

    /// 0x6xkk
    /// "LD Vx, byte" opcode. Set Vx = kk.
    fn opcode_ld_vx_byte(args: OpCodeArgs, cpu: Cpu) {

    }

    /// 0x7xkk
    /// "ADD Vx, byte" opcode. Set Vx = Vx + kk.
    fn opcode_add_vx_byte(args: OpCodeArgs, cpu: Cpu) {

    }

    /// 0x8xy0
    /// "LD Vx, Vy" opcode. Set Vx = Vy.
    fn opcode_ld_vx_vy(args: OpCodeArgs, cpu: Cpu) {

    }

    /// 0x8xy1
    /// "OR Vx, Vy" opcode. Set Vx = Vx OR Vy.
    fn opcode_or_vx_vy(args: OpCodeArgs, cpu: Cpu) {

    }

    /// 0x8xy2
    /// "AND Vx, Vy" opcode. Set Vx = Vx AND Vy.
    fn opcode_and_vx_vy(args: OpCodeArgs, cpu: Cpu) {

    }

    /// 0x8xy3
    /// "XOR Vx, Vy" opcode. Set Vx = Vx XOR Vy.
    fn opcode_xor_vx_vy(args: OpCodeArgs, cpu: Cpu) {

    }

    /// 0x8xy4
    /// "ADD Vx, Vy" opcode. Set Vx = Vx + Vy, set VF = carry.
    fn opcode_add_vx_vy(args: OpCodeArgs, cpu: Cpu) {

    }

    /// 0x8xy5
    /// "SUB Vx, Vy" opcode. Set Vx = Vx - Vy, set VF = NOT borrow.
    fn opcode_sub_vx_vy(args: OpCodeArgs, cpu: Cpu) {

    }

    /// 0x8xy6
    /// "SHR Vx, Vy" opcode. Set Vx = Vy SHR 1.
    fn opcode_shr_vx_vy(args: OpCodeArgs, cpu: Cpu) {

    }

    /// 0x8xy7
    /// "SUBN Vx, Vy" opcode. Set Vx = Vy - Vx, set VF = NOT borrow.
    fn opcode_subn_vx_vy(args: OpCodeArgs, cpu: Cpu) {

    }

    /// 0x8xyE
    /// "SHL Vx, Vy" opcode. Set Vx = Vy SHL 1.
    fn opcode_shl_vx_vy(args: OpCodeArgs, cpu: Cpu) {

    }

    /// 0x9xy0
    /// "SNE Vx, Vy" opcode. Skip next instruction if Vx != Vy.
    fn opcode_sne_vx_vy(args: OpCodeArgs, cpu: Cpu) {

    }

    /// 0xAnnn
    /// "LD I, addr" opcode. Set I = nnn.
    fn opcode_ld_i_addr(args: OpCodeArgs, cpu: Cpu) {

    }

    /// 0xBnnn
    /// "JP V0, addr" opcode. Jump to location nnn + V0.
    fn opcode_jp_v0_addr(args: OpCodeArgs, cpu: Cpu) {

    }

    /// 0xCxkk
    /// "RND Vx, byte" opcode. Set Vx = random byte AND kk.
    fn opcode_rnd_vx_byte(args: OpCodeArgs, cpu: Cpu) {
